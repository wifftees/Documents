...
+ 
  - Покажем, что $M = {f_a | forall a in A} $ есть базис $F(A)$
  $
    forall f in F(A): forall x in A: f(x) = f(x) f_x (x) #h(0.5cm) f_x in M \    
  $
  - Для любого $x$ из области определения $f$ существует образ $xi = f(x)$, который можно
    назвать коэффициентом, и существует $f_x in M$, тогда $f$ на любом значении однозначно выражается через $M$ и коэффициент.
